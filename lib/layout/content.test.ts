...
describe('TextNodeSchema', () => {
  it('accepts a minimal valid text node', () => {
    expect(TextNodeSchema.parse({ type: 'text', role: 'headline', content: 'hello' }).type).toBe(
      'text',
    );
  });
  ...
  it('rejects empty content', () => {
    expect(TextNodeSchema.safeParse({ type: 'text', role: 'body', content: '' }).success).toBe(
      false,
    );
  });
  it('rejects bad role', () => {
    expect(TextNodeSchema.safeParse({ type: 'text', role: 'wow', content: 'x' }).success).toBe(
      false,
    );
  });
  it('rejects bad align', () => {
    expect(
      TextNodeSchema.safeParse({ type: 'text', role: 'body', content: 'x', align: 'left' }).success,
    ).toBe(false);
  });
  it('rejects extra keys', () => {
    expect(
      TextNodeSchema.safeParse({ type: 'text', role: 'body', content: 'x', extra: 1 }).success,
    ).toBe(false);
  });
...
describe('ImageNodeSchema', () => {
  ...
  it('rejects bad aspect', () => {
    expect(
      ImageNodeSchema.safeParse({ type: 'image', brief: 'b', alt: 'a', aspect: '2:1' }).success,
    ).toBe(false);
  });
  it('rejects bad assetUrl', () => {
    expect(
      ImageNodeSchema.safeParse({ type: 'image', brief: 'b', alt: 'a', assetUrl: 'not-url' })
        .success,
    ).toBe(false);
  });
  it('rejects focal out of range', () => {
    expect(
      ImageNodeSchema.safeParse({ type: 'image', brief: 'b', alt: 'a', focal: { x: 101, y: 0 } })
        .success,
    ).toBe(false);
  });
...
describe('ButtonNodeSchema', () => {
  ...
  it('accepts variant', () => {
    expect(
      ButtonNodeSchema.parse({ type: 'button', label: 'Go', href: '/x', variant: 'ghost' }).variant,
    ).toBe('ghost');
  });
  it('rejects empty label', () => {
    expect(ButtonNodeSchema.safeParse({ type: 'button', label: '', href: '/x' }).success).toBe(
      false,
    );
  });
  it('rejects label over 80', () => {
    expect(
      ButtonNodeSchema.safeParse({ type: 'button', label: 'x'.repeat(81), href: '/x' }).success,
    ).toBe(false);
  });
  it('rejects bad variant', () => {
    expect(
      ButtonNodeSchema.safeParse({ type: 'button', label: 'x', href: '/x', variant: 'bold' })
        .success,
    ).toBe(false);
  });
});
...
describe('WordmarkNodeSchema', () => {
  it('accepts text kind', () => {
    expect(WordmarkNodeSchema.parse({ type: 'wordmark', kind: 'text', content: 'Shop' }).kind).toBe(
      'text',
    );
  });
  it('rejects bad kind', () => {
    expect(
      WordmarkNodeSchema.safeParse({ type: 'wordmark', kind: 'svg', content: 'x' }).success,
    ).toBe(false);
  });
  it('rejects empty content', () => {
    expect(
      WordmarkNodeSchema.safeParse({ type: 'wordmark', kind: 'text', content: '' }).success,
    ).toBe(false);
  });
});
...
describe('VideoNodeSchema', () => {
  it('accepts minimum', () => {
    expect(VideoNodeSchema.parse({ type: 'video', assetUrl: 'https://e.com/v.mp4' }).type).toBe(
      'video',
    );
  });
  it('accepts full', () => {
  ...
  it('rejects bad poster', () => {
    expect(
      VideoNodeSchema.safeParse({ type: 'video', assetUrl: 'https://e.com/v', poster: 'x' })
        .success,
    ).toBe(false);
  });
});
...
describe('DividerNodeSchema', () => {
  ...
  it('accepts weight + style', () => {
    expect(
      DividerNodeSchema.parse({ type: 'divider', weight: 'thick', style: 'dotted' }).weight,
    ).toBe('thick');
  });
...
describe('QuoteNodeSchema', () => {
  ...
  it('accepts attribution + role', () => {
    expect(
      QuoteNodeSchema.parse({ type: 'quote', body: 'A', attribution: 'Bo', role: 'Founder' })
        .attribution,
    ).toBe('Bo');
  });
...
describe('ProductGridNodeSchema', () => {
  ...
  it('rejects bad order', () => {
    expect(ProductGridNodeSchema.safeParse({ type: 'productGrid', order: 'random' }).success).toBe(
      false,
    );
  });
  it('rejects mobileColumns over 3', () => {
    expect(ProductGridNodeSchema.safeParse({ type: 'productGrid', mobileColumns: 4 }).success).toBe(
      false,
    );
  });
  it('rejects empty filter string', () => {
    expect(
      ProductGridNodeSchema.safeParse({ type: 'productGrid', filter: { collectionSlug: '' } })
        .success,
    ).toBe(false);
  });
  it('rejects manualIds > 48', () => {
    expect(
      ProductGridNodeSchema.safeParse({ type: 'productGrid', manualIds: Array(49).fill('a') })
        .success,
    ).toBe(false);
  });
});
...
describe('FeaturedProductNodeSchema', () => {
  it('accepts', () => {
    expect(
      FeaturedProductNodeSchema.parse({ type: 'featuredProduct', productId: 'p1' }).productId,
    ).toBe('p1');
  });
  it('accepts full', () => {
    expect(
  ...
  it('rejects empty productId', () => {
    expect(
      FeaturedProductNodeSchema.safeParse({ type: 'featuredProduct', productId: '' }).success,
    ).toBe(false);
  });
});
...
describe('CollectionGridNodeSchema', () => {
  ...
  it('rejects count > 24', () => {
    expect(CollectionGridNodeSchema.safeParse({ type: 'collectionGrid', count: 25 }).success).toBe(
      false,
    );
  });
  it('rejects bad order', () => {
    expect(
      CollectionGridNodeSchema.safeParse({ type: 'collectionGrid', order: 'price-asc' }).success,
    ).toBe(false);
  });
});
...
describe('FeaturedCollectionNodeSchema', () => {
  it('accepts minimum', () => {
    expect(
      FeaturedCollectionNodeSchema.parse({ type: 'featuredCollection', collectionSlug: 'wax' })
        .collectionSlug,
    ).toBe('wax');
  });
  it('accepts previewCount', () => {
    expect(
      FeaturedCollectionNodeSchema.parse({
  ...
  it('rejects previewCount > 12', () => {
    expect(
      FeaturedCollectionNodeSchema.safeParse({
        type: 'featuredCollection',
        collectionSlug: 'w',
        previewCount: 13,
      }).success,
...
describe('SubscriptionGridNodeSchema', () => {
  it('accepts minimum', () => {
    expect(SubscriptionGridNodeSchema.parse({ type: 'subscriptionGrid' }).type).toBe(
      'subscriptionGrid',
    );
  });
  it('rejects count > 12', () => {
    expect(
      SubscriptionGridNodeSchema.safeParse({ type: 'subscriptionGrid', count: 13 }).success,
    ).toBe(false);
  });
  it('rejects columns > 4', () => {
    expect(
      SubscriptionGridNodeSchema.safeParse({ type: 'subscriptionGrid', columns: 5 }).success,
    ).toBe(false);
  });
  it('rejects mobileColumns > 2', () => {
    expect(
      SubscriptionGridNodeSchema.safeParse({ type: 'subscriptionGrid', mobileColumns: 3 }).success,
    ).toBe(false);
  });
});
...
describe('FeaturedSubscriptionNodeSchema', () => {
  it('accepts', () => {
    expect(
      FeaturedSubscriptionNodeSchema.parse({ type: 'featuredSubscription', subscriptionId: 's1' })
        .subscriptionId,
    ).toBe('s1');
  });
  it('rejects empty id', () => {
    expect(
      FeaturedSubscriptionNodeSchema.safeParse({ type: 'featuredSubscription', subscriptionId: '' })
        .success,
    ).toBe(false);
  });
});
  ...
  it('rejects bad field kind', () => {
    expect(
      ContactFormNodeSchema.safeParse({
        type: 'contactForm',
        fields: [{ name: 'x', label: 'X', kind: 'password' }],
      }).success,
    ).toBe(false);
  });
  it('rejects empty fields array', () => {
    expect(ContactFormNodeSchema.safeParse({ type: 'contactForm', fields: [] }).success).toBe(
      false,
    );
  });
...
describe('SocialLinksNodeSchema', () => {
  ...
  it('rejects bad style', () => {
    expect(SocialLinksNodeSchema.safeParse({ type: 'socialLinks', style: 'big' }).success).toBe(
      false,
    );
  });
  it('rejects > 12 platforms', () => {
    expect(
      SocialLinksNodeSchema.safeParse({ type: 'socialLinks', platforms: Array(13).fill('x') })
        .success,
    ).toBe(false);
  });
});
...
describe('EventsListNodeSchema', () => {
  ...
  it('rejects bad layout', () => {
    expect(EventsListNodeSchema.safeParse({ type: 'eventsList', layout: 'masonry' }).success).toBe(
      false,
    );
  });
  it('rejects count > 24', () => {
