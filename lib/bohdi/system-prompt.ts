...
// The legacy block-and-tokens prompt is the only Bohdi prompt now. The
// layout-engine path (INTRO_MOMENT_PROMPT / LAYOUT_ENGINE_PROMPT) was deleted
// once every build moved to the crew — it was unreached and carried
// niche-example bias we strip on sight.
export function systemPromptFor(_nicheSlug: string): string {
