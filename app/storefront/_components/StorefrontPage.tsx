...
export default async function StorefrontPage({ slug, version, previewLook, previewHero, previewGoods }: StorefrontPageProps) {
  ...
  if (
    ...
    return renderArchetypeStore(rootRaw as Record<string, unknown>, tenantId, undefined, previewLook, previewHero, previewGoods);
  }
...
async function renderArchetypeStore(env: Record<string, unknown>, tenantId: string, page?: ArchetypePage, overrideLook?: string, previewHero?: string, previewGoods?: string) {
  ...
  const accentOverride = typeof env['accentOverride'] === 'string' ? (env['accentOverride'] as string) : undefined;
  const { logoUrl, brandColors } = await loadTenantChrome(tenantId);
  return spec.render({ content: env['content'], lookKey: effectiveLook, products, mood, catalogSize, page, logoUrl, brandColors, accentOverride, tenantId, heroVariant: previewHero, goodsTreatment: previewGoods });
}
