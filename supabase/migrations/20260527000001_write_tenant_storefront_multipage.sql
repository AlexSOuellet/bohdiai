-- Extend write_tenant_storefront to accept multiple pages in one atomic write.
-- Input shape changes from { ..., blocks, listings } to { ..., pages: [{slug, pageType, title, blocks}], listings }.
-- Used by the multi-page generation pipeline (home + /shop + /contact, optionally /gallery).
...
begin
  ...
  -- 3. Pages + page_blocks (one transaction, multiple pages)
  ...
  -- 4. Listings (optional — may be empty)
...
comment on function write_tenant_storefront(jsonb) is
  'Atomically creates a tenant + all onboarding artefacts (multiple pages) in one transaction. Called from lib/generation/write-storefront.ts.';
