-- Atomic write for layout-engine tenants.
-- Mirrors write_tenant_storefront but writes style_sheets + content_pages with
-- layout_tree (no design_tokens, no page_blocks).
-- Project-Docs/Layout-Language.md.
...
begin
  ...
  -- 3. Pages with layout_tree (no page_blocks)
  ...
  loop
    insert into content_pages (
      ...
      is_system_page, is_in_nav, layout_tree
    )
    values (
      v_tenant_id,
      v_page->>'slug',
...
comment on function write_tenant_storefront_layout(jsonb) is
  'Atomic write for layout-engine tenants — tenant + style_sheet + content_pages with layout_tree + collections + listings + subscriptions. Companion to write_tenant_storefront. Called from lib/generation/write-storefront-layout.ts.';
