...
interface NavCenteredWordmarkProps {
  content: NavCenteredWordmarkContent;
}
...
export default function NavCenteredWordmark({ content }: NavCenteredWordmarkProps) {
  ...
  // Split links symmetrically — first half left of the wordmark, second half right.
  const mid = Math.ceil(validLinks.length / 2);
  const leftLinks = validLinks.slice(0, mid);
  const rightLinks = validLinks.slice(mid);
  ...
  return (
    <header className={headerClass}>
      ...
      {/* ─── Desktop: links split around centered wordmark ─── */}
      <div className="hidden md:grid max-w-7xl mx-auto px-6 grid-cols-[1fr_auto_1fr] items-center gap-8">

        {/* Left links */}
        <nav className="flex items-center justify-end gap-8" aria-label="Primary navigation">
          {leftLinks.map((key) => {
        ...
        {/* Right links + cart */}
        <nav className="flex items-center justify-start gap-8" aria-label="Secondary navigation">
          {rightLinks.map((key) => {
            const link = SECTION_LINKS[key];
            if (link === undefined) return null;
            return <a key={key} href={link.href} className={linkClass}>{link.label}</a>;
          })}
          <a href="/cart" aria-label="Cart" className="text-s-text/65 hover:text-s-text transition-colors duration-200 ml-auto">
            <CartIcon />
          </a>
        </nav>
      ...
      {/* ─── Mobile: wordmark centered, cart right ─── */}
      <div className="md:hidden grid grid-cols-[1fr_auto_1fr] items-center px-6">
