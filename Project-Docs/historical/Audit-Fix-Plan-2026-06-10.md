...
- [ ] **D1. Quick-wins batch:** fix the open redirect in `app/auth/callback/route.ts:15` (require a single leading `/`); fix the raw `<a>` in `lib/archetypes/main-street/chrome.tsx:240`. (The stale `'simple'` mood key moved to D5b — it's part of the legacy cluster, and the whole mood list is out of date, not just one word; not a standalone quick fix.)
...
- [ ] **D3a. (corrected) The `/gallery` route is NOT a standalone delete.** It's still wired into the storefront nav — `storefront-chrome.ts` queries for a `/gallery` page and links it (`:42`, `:68`). Deleting the route alone would break that link. It belongs to the legacy rendering cluster — handle in D5b.
...
- [ ] **D5b. The legacy rendering cluster — verify, then remove together.** After deleting the old test stores (2026-06-10) there are now **0 legacy pages and 0 `style_sheets` rows** — the old path serves no data. But the *code* is one tangled unit: the `StorefrontPage`/`LayoutPage` fallback, the `style_sheets` read (`StorefrontPage.tsx:226`), the `/gallery` route + its `storefront-chrome` nav query (D3a), and the stale mood list (`main-street/index.tsx:26`). It likely **doubles as the renderer for maker-added custom pages** (D37), so it is NOT confirmed-dead. **Next step:** verify whether custom pages / any fallback still need that path; if not, remove the whole cluster in one pass — `style_sheets` table, the fallback renderer, the gallery route + nav query, and the stale mood metadata. A real verify-then-decide, not a mechanical drop.
