...
**Session 73 settled AND built the niche-texture system that stalled in Session 72.** Editor Door 2's texture picker is live: a maker picks a texture from their niche's curated shelf and it applies to the storefront preview, keeping their own background color. Candle-maker is the first niche with a real shelf. Alex live-tested on Aurora Candles and confirmed before commit.

The settled mechanism: a niche texture is a **transparent alpha PNG** (pattern in alpha, sourced from stock then processed once with `sharp`, NOT AI-generated) that **blends** onto the maker's background rather than repainting it — `multiply` on light families (pattern deepens the color), invert+`screen` on dark families (pattern lightens it), chosen per family from bg luminance. The background color is always preserved; the texture only adds its own shadow/highlight. Curation lives in the niche style sheet (`content/style-sheets/niche-<slug>.json`, `textures: [{key,name,sourceUrl,...}]`) — the maker can only pick from what we ship, same guardrail as fonts/palette. Preview-only for now (URL params); no `commitTexture` to persist yet.

Candle shelf: **three that work** (Woven Linen, Aged Glaze, Marble Swirl — a grid, cracks, swirls, structurally distinct) and **two flagged weak** (Handmade Paper, Cotton Tooth — process to ~2-3% ink, near-invisible). All five are on the sheet; Alex judges whether to drop or replace the two. Curation lesson banked: sameness is STRUCTURAL, not opacity — pick different structures, not five grainy papers.
...
3. **Revise the niche-writer skill's textures section** (still carries the Session-72 REVISION PENDING banner). The shape is now settled: object entries `{ key, name, sourceUrl }`, sourced-not-generated, transparent alpha PNG, blend-not-fill. Update so cowork can produce shelves for other niches; then decide whether to backfill the 7 existing sheets.
...
- Session 73 (2026-07-16): Settled AND built the niche-texture system that stalled in Session 72. Editor Door 2 texture picker is live and preview-wired: a tenant's `primary_niche` loads its texture shelf from `content/style-sheets/niche-<slug>.json`; the maker picks (Family default / No texture / shelf entry) and the storefront preview re-renders via `previewTexture` + `previewTextureOpacity` URL params. Settled mechanism: texture is a TRANSPARENT ALPHA PNG (pattern-in-alpha, black RGB), sourced from stock then processed once with `sharp` (grayscale→negate→alpha), NOT AI-generated; it BLENDS onto the maker's background instead of repainting — `multiply` on light families (deepen), invert+`screen` on dark families (lighten), mode chosen per family from bg luminance (`hexIsDark`, 0.4). Background color always preserved. First shelf: candle-maker, 5 entries — 3 that work (Woven Linen grid / Aged Glaze cracks / Marble Swirl veins, structurally distinct) + 2 flagged weak (Handmade Paper, Cotton Tooth, ~2-3% ink). New files: `niche-candles.json`, 5 processed PNGs in `public/textures/niche/candles/`, `lib/editor/load-niche-textures.ts` (+test), `ViewLiveSiteLink.tsx` (reused live-site tab carrying try-on via localStorage). Preview-only — no `commitTexture` persist yet. 948 tests pass (+8). tsc+lint clean. Alex live-tested on Aurora Candles, confirmed multiply preserves each palette's color, then approved commit. Long frustrating path to get there — full wrong-turns record (JPG→soft-light→fixed-ink PNG→fg-mask→alpha-blend) and process lessons (stop when told, don't self-grade curation, don't race Alex's dev server, don't reset his password) in the session log.
