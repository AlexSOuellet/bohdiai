...
- [x] **B2.** Shop-section teaser CTA button label — root cause was the shared `[data-type="navLabel"]` rule capping every navLabel to `max-width:240px` + nowrap + ellipsis. Top nav labels need that clamp, but the pill CTAs are authored longer copy that should wrap not truncate. Fix kept the base clamp for actual nav labels and overrode on `.ms-shopcue-btn` + `.ms-close-cta`: `max-width:36ch`, `white-space:normal`, `overflow:visible`, `text-overflow:clip`, `text-align:center`. Class specificity beats the attribute selector without `!important`. Shipped commit `93c53c3` in Session 67; checkbox flip missed at the time (caught Session 70). Alex verified live.
...
- [x] **CLOSED — contrast pairs are authored per skin.** The doc entry was written before the pairs landed. Every one of the 29 skins in `skins.ts` declares an explicit `contrast: { bg, fg, fgMuted }` — not a fallback inversion. The pairs are largely family-appropriate: Rustic flips warm cream to deep plum, Cozy Sprout flips cream to forest green, another Cozy flips near-white to deep aubergine, Cheerful skins flip cream to deep purple / teal / electric blue / oxblood / deep teal. A handful of dark-based skins (Hearthstone, Gild, etc.) do use a light contrast surface, which for genuinely-dark skins is the correct "flip to light" moment, not the "generic white and charcoal" pattern the doc worried about. Verified Session 70 by reading `skins.ts` directly.
