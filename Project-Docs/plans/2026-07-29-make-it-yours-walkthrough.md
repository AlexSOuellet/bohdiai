...
5. **Personal content needs the maker's real input; the rest Bohdi can write; the maker can always write any field themselves (D68).** The About / story / founder line are written only from what the maker tells us — the About/story step asks several targeted questions to gather the real material, and there is **no** "write it from a feeling" shortcut for personal content; if the maker gives nothing, the section stays honestly unfinished. Everything else (headlines, hero lines, taglines, headings, CTAs, product descriptions, marquee) Bohdi can write from a direction / feeling / niche. On **every** field, "type exactly what I want" is a first-class path, not just a tweak-after. Reviews are real quotes or switched off — never AI-generated.
...
Each step names a section, the text field ids it rewrites, an optional image slot id, and whether it's an optional-section step. Per D68 a step also carries `personal: boolean`: the story/founder beat is `personal` and carries the targeted questions that gather the maker's real material (how it started, what they make, what makes theirs theirs, who it's for) — personal steps have **no** feeling-only generation. "Placeholder" = a section not in `content.madeYours` and not hidden (we mark `madeYours` on any edit/upload/toggle — pragmatic completeness without per-field diffing).
...
Per step (design Part 3, D68): Bohdi *leads* with a plain-language ask; the maker can **type their own words directly** (first-class on every field, taken as-is), or — on non-personal steps only — give a direction/feeling and let Bohdi write; **personal steps (story/founder) require the maker's input** and gather it through the step's targeted questions (no feeling-only). Submit calls `editContent(step.fieldIds, input, step.section)`; steps with `imageSlot` also render `<ImagePicker slotId={step.imageSlot} onUploaded={...}/>`; the preview iframe refreshes after any change; the maker can **keep** (next), **try again** (re-run same input), or **tweak** (direct edit); optional-section steps also offer **switch it off** → `toggleSection(section,false)` and advance. Progress cue ("N of M made yours"). Undo/Reset/Publish stay (reuse the Editor's controls).
