...
**Videos are generated by Cowork.** The three hero videos use Kling 3.0 Turbo and cost ~4.5 credits each, which is not covered by the unlimited plan, so Cowork generates them directly — ideally animating the niche's hero image as the start frame so the clip matches the still.
...
- **3 hero videos.** 16:9 at 720p, 3 seconds. IN-FRAME motion only — a flame flickering, steam rising, hands turning a piece. LOCKED CAMERA — no pans, no push-ins, no rack focus, no zoom. A moving camera breaks the seamless loop.
...
**Video-specific:** locked camera only. In-frame motion only. Loop-safe. State the locked-camera rule explicitly in the prompt.
...
Post each generation to `POST /api/library/ingest` with a bearer token. The endpoint downloads the binary, uploads to Supabase Storage, inserts a row with `approved=false`.

Bearer token: read from environment variable `COWORK_INGEST_TOKEN`. Alex sets this on the machine you run in.
