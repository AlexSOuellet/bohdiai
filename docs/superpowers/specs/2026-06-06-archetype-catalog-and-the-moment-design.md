...
- **Melt vs gate.** The Moment *melts into the page as the hero* only when the archetype already has a hero surface for it to become (Main Street). Otherwise it is a *hard gate* that plays, then dismisses ("enter shop") to reveal the archetype's shape (Counter, Find, and The One's launch use).
- **Cookie / replay.** A **per-shop** cookie marks the Moment seen. First front-door visit → it plays, cookie set. Returning front-door visit with the cookie → straight to the shape.
